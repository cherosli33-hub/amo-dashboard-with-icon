...
const ASSETS = [
  ...
  "./data-dashboard/modules/phc-findings.js",
  "./data-dashboard/modules/girn-findings.js",
  "./data-dashboard/modules/supervisor-audit.js",
  "./manifest.json?v=4",
  "./icon-192.png?v=4",
  "./icon-512.png?v=4",
...
// PHC Checklist ada service worker sendiri dalam ./phc-checklist/.
// SW portal ini mesti tidak memintas atau cache apa-apa di bawah laluan tersebut.
function isPhcScope(request){
  return new URL(request.url).pathname.includes("/phc-checklist/");
}

self.addEventListener("fetch", event => {
  if(event.request.method !== "GET" || new URL(event.request.url).origin !== self.location.origin) return;
  if(isPhcScope(event.request)) return;
