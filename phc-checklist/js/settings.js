...
const copy={
  ...
  about:["Tentang aplikasi",`PHC Checklist versi ${APP_VERSION}. Data dihantar ke Firebase dan salinan sementara disimpan pada peranti untuk kegunaan offline.`],
};
...
document.querySelector("#clearCache").addEventListener("click",()=>{ if(confirm("Kosongkan salinan rekod pada peranti ini? Data yang sudah dihantar ke Firebase tidak akan dipadam.")){ localStorage.removeItem(STORAGE_KEY); localStorage.removeItem(LATEST_INVENTORY_KEY); if(!loadPendingSync().length) localStorage.removeItem(PENDING_SYNC_KEY); alert("Cache peranti telah dikosongkan."); location.href="index.html"; } });

async function showStatus(){
  const pending=loadPendingSync().length;
  if(!apiConfigured()){ status.className="sync-card pending"; status.innerHTML=`<strong>Firebase belum disambungkan</strong><p>${pending} rekod menunggu sync. Masukkan URL Apps Script dalam konfigurasi aplikasi.</p>`; return; }
  ...
  status.innerHTML=`<strong>${result.pending?"Ada rekod menunggu sync":"Firebase tersambung"}</strong><p>${result.synced} rekod baru dihantar · ${result.pending} masih menunggu.</p>`;
}
showStatus();
